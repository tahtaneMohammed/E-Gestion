...
export default function SemesterAbsencesPage() {
  ...
  useEffect(() => {
    const loadData = () => {
      ...
      try {
        ...
        if (savedData) {
          ...
          if (isRecent) {
            ...
            toast({
              ...
              description: "تم تحميل بيانات الأساتذة والطلاب من ملف الإكسل المستورد",
            })
          } else {
        ...
        const savedAbsencesData = localStorage.getItem("semesterAbsences")
  ...
  const markStudentAbsent = (student: Student) => {
    const newAbsence: AbsenceRecord = {
      ...
      type: "student",
      notes: `القسم: ${student.class}`,
    }

    const updatedAbsences = [...savedAbsences, newAbsence]
    setSavedAbsences(updatedAbsences)
    localStorage.setItem("semesterAbsences", JSON.stringify(updatedAbsences))
  ...
  const generateReport = () => {
    ...
    try {
      ...
      const reportTitle = "محضر الغيابات - الامتحانات الفصلية"
      let reportSubtitle = ""
      ...
      if (pdfType !== "all") {
        reportSubtitle += `النوع: ${pdfType === "students" ? "الطلاب" : "الأساتذة"}`
      }
      ...
      <html dir="rtl">
      <head>
        <title>محضر الغيابات - الامتحانات الفصلية</title>
        <meta charset="UTF-8">
        <style>
      ...
      <body>
        <div class="header">
          ...
          <h2>${basicSettings.institutionName || "ثانوية الأمير عبد القادر"}</h2>
        </div>
        
        <div class="info">
          <div>
            <div class="info-item"><strong>الامتحانات الفصلية</strong></div>
            <div class="info-item"><strong>المؤسسة:</strong> ${basicSettings.institutionName || "ثانوية الأمير عبد القادر"}</div>
            <div class="info-item"><strong>السنة الدراسية:</strong> ${basicSettings.academicYear || "2024-2025"}</div>
          </div>
          <div>
        ...
        <div class="stats-container">
          <div class="stat-box">
            ...
            <div class="stat-label">غياب الطلاب</div>
          </div>
          <div class="stat-box">
            <div class="stat-value">${studentLateCount}</div>
            <div class="stat-label">تأخر الطلاب</div>
          </div>
          <div class="stat-box">
        ...
        ${
          pdfType === "all" || pdfType === "students"
            ? `
        <div class="section-subtitle">سجل غياب الطلاب</div>
          ...
          <tbody>
            ${studentAbsences
              ...
              <tr>
                <td>${index + 1}</td>
                <td>${absence.name}</td>
                <td>${absence.notes?.replace("القسم: ", "") || "-"}</td>
                <td>${absence.date}</td>
        ...
        `
            : '<div class="no-data">لا توجد غيابات مسجلة للطلاب</div>'
        }
        `
            : ""
        }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        <header className="flex items-center mb-6">
          ...
          <h1 className="text-2xl font-bold mr-4">تسيير الغيابات - الامتحانات الفصلية</h1>
        </header>
